...
{
    public partial class TransferOrderCancelRequest : System.Web.UI.Page
    {
        ...
        protected void cancelRequestLinkButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                productTransferOrder.UpdateProductTransferOrderStatusById(transferOrderListGridView.Rows[row.RowIndex].Cells[0].Text.ToString(), "CP");

                MyAlertBox("alert(\"Product Transfer Order [" + transferOrderListGridView.Rows[row.RowIndex].Cells[0].Text.ToString() + "] Cancel Request Submitted Successfully.\"); window.location=\"/UI/ProductTransferOrder/TransferOrderCancelRequest.aspx\"");
            }
            catch (Exception ex)
            {
                msgbox.Visible = true; msgTitleLabel.Text = "Exception!!!"; msgDetailLabel.Text = ex.Message;
