            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!warehouse.CheckDuplicateWarehouse(warehouse.WarehouseName.Trim()))
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            string message = "Business <span class='actionTopic'>Created</span> Successfully with Business ID: <span class='actionTopic'>" + dt.Rows[0][0].ToString() + "</span>.";
                            MyAlertBox("var callbackOk = function () { MyOverlayStart(); window.location = \"/UI/business/List.aspx\"; }; SuccessAlert(\"" + "Process Succeed" + "\", \"" + message + "\", callbackOk);");
                        }
                        else
                        {
                            string message = "<span class='actionTopic'>Failed</span> to Create Business.";
                            MyAlertBox("ErrorAlert(\"" + "Process Failed" + "\", \"" + message + "\");");
                        }
                    }
                    else
                    {
                        string message = "This Business <span class='actionTopic'>already exist</span>, try another one.";
