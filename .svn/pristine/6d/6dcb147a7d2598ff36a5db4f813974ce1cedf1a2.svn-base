...
namespace lmxIpos.UI.PurchaseRecord
{
    public partial class CreatePurchase : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        //LoadVendors();
                        vendorDropDownList.SelectedItem.Text = dt.Rows[0]["VendorName"].ToString();
        ...
        protected void LoadVendors()
        {
            ...
            try
            {
                vendor.WarehouseId = warehouseIdLabel.Text;
        ...
        protected void saveButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < purchaseOrderProductListGridView.Rows.Count; i++)
                {
                    ...
                    if (!decimal.TryParse(purchaseQuantityTextBox.Text.Trim(), out num))
                    {
                        msg = "Product ID [" + purchaseOrderProductListGridView.Rows[i].Cells[0].Text.ToString() + "] has no valid Purchase Qty.";
                    ...
                    {
                        msg = "Product ID [" + purchaseOrderProductListGridView.Rows[i].Cells[0].Text.ToString() + "] has no valid Rate Per Unit.";
                        msgbox.Visible = true; msgTitleLabel.Text = "Exception!!!"; msgDetailLabel.Text = msg;
                        return;
                    }
                    else if (!float.TryParse(amountTextBox.Text.Trim(), out fnum))
                    {
                        msg = "Product ID [" + purchaseOrderProductListGridView.Rows[i].Cells[0].Text.ToString() + "] has no valid Amount.";
                        msgbox.Visible = true; msgTitleLabel.Text = "Exception!!!"; msgDetailLabel.Text = msg;
                        return;
                    }
                    else if (!float.TryParse(unitPriceTextBox.Text.Trim(), out uprice))
                    {
                        msg = "Product ID [" + purchaseOrderProductListGridView.Rows[i].Cells[0].Text.ToString() + "] has no valid Unit Price.";
                        msgbox.Visible = true; msgTitleLabel.Text = "Exception!!!"; msgDetailLabel.Text = msg;
                        return;
                    }
                    else
                    {
                        ...
                        dr["ProductId"] = purchaseOrderProductListGridView.Rows[i].Cells[0].Text.ToString();
                        ...
                        string[] oQty = purchaseOrderProductListGridView.Rows[i].Cells[3].Text.ToString().Split(' ');

                        if (decimal.Parse(purchaseQuantityTextBox.Text.Trim()) == 0)
                        {
                ...
                if (dt.Rows.Count == purchaseOrderProductListGridView.Rows.Count)
                {
                    ...
                    purchaseRecord.VendorId = lblVendorID.Text.Trim();
